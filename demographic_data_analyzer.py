...
def calculate_demographic_data(print_data=True):
    ...
    perc = len(lower_education)
    pt = len(df)
    percentage = (perc/pt)*100
    df = df.round(10)
    ...
    rich_percentage = min_work_hours = df['hours-per-week'].min()
    num_min_workers = df[df['hours-per-week'] == min_work_hours]
    ...
    total_population_by_country = country_group.size()
    rich_population_by_country = country_group.apply(lambda x: (x['salary'] == '>50K').sum())
    percentage_rich_by_country = (rich_population_by_country / total_population_by_country) * 100
    
    highest_earning_country = percentage_rich_by_country.idxmax()
    country_group = df.groupby('native-country')
    rich_percentage_by_country = (country_group.apply(lambda x: (x['salary'] == '>50K').sum()) / country_group.size()) * 100
